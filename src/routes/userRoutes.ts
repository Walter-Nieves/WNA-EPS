...
import { actualizarUsuario, crearUsuario, eliminarUsuario, obtenerUsuarioActual, obtenerUsuarioPorId, obtenerUsuarios } from '../controller/userController'
...
userRoutes.get('/', authVerify, obtenerUsuarios)

// 🔹 Obtener usuario autenticado (/me)
userRoutes.get('/me', authVerify, obtenerUsuarioActual)

// 🔹 Obtener usuario por ID o cédula
userRoutes.get('/:id', authVerify, obtenerUsuarioPorId)
...
userRoutes.post('/', subir.single('foto'), crearUsuario)
...
userRoutes.put('/:id', subir.single('foto'), actualizarUsuario)

// 🔹 Eliminar usuario
userRoutes.delete('/:id', eliminarUsuario)
...
// // buscar personas
// userRoutes.get('/', authVerify, async (_, res) => {
...
// userRoutes.get('/me', authVerify, async (_, res) => {
//   try {
//     const resultado = await ColUsuarios.findOne({ _id: new ObjectId(res.locals.usuario.sub) }, { projection: { clave: false } }) as Usuario
//     if (resultado == null) {
//       resError(404, 'Usuario no encontrado')
//     }
//     res.json(resultado)
//   } catch (error) {
...
// // Buscar persona por _id o por cédula especifico
...
// userRoutes.post('/', subir.single('foto'), async (req, res) => {
//   // const body = req.body
//   try {
...
//     const sal = await bcrypt.genSalt(12)// generar sal(12) significa que se aplicarán 2^12 rondas de procesamiento para generar la sal.
...
// userRoutes.put('/:id', subir.single('foto'), async (req, res) => {
//   try {
...
//       return resError(500, 'No se pudo actualizar el usuario') // Esto indica que algo salió mal en la actualización.
//     }
