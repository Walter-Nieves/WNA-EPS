...
function Campo<T extends FieldValues>({tipo,placeholder,regis,errors,nombre}:CampoProps<T>) {
  ...
  return (
    <div>
        ...
        type={tipo == "password" ? !mostrarClave ? "password" : "text" : tipo} placeholder={placeholder}{...regis} />
        {tipo == "password" && <button type="button" onClick={()=>setMostrarClave(!mostrarClave)} className="pl-1 cursor-pointer absolute  top-4 right-2" >
            ...
            {errors[nombre] && <p className="text-red-500 text-[10px] flex justify-start w-full">{errors[nombre]?.message?.toString()}</p>}
    </div>
  )
}
